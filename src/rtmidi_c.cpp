...
template<bool b>
class StaticEnumAssert {
private:
    StaticEnumAssert() {
    }
};
template<>
class StaticEnumAssert<true> {
public:
    StaticEnumAssert() {
    }
};
...
public:
    CallbackProxyUserData(RtMidiCCallback cCallback, void *userData) : c_callback(cCallback), user_data(userData) {
    }
...
static void callback_proxy(double timeStamp, std::vector<unsigned char> *message, void *userData) {
